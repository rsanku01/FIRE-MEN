...
const char* mqtt_server = "10.20.29.212";                                   //get the ifconfig and change it
...
void setup_wifi() {
  delay(10);
  WiFi.begin ("udcdocencia", "Universidade.2022");                        //connect the board with internet
...
void callback (char* topic, byte* payload, unsigned int length) {                    //loop of the callbacks

  
  if (String(topic) == topic_update_parameters) {                                    //if command to update parameters
    char inmsg[length + 1];
  ...
  if (String(topic) == topic_get_parameters) {                                       //if command to get the parameters
  ...
  if (String(topic) == topic_on_off) {                                              //if command to publish on and off 
  ...
  if (String(topic) == topic_data) {                                               //if command to subscribe to the data
  ...
  if (String(topic) == topic_parameters) {                                       //if command to subscribe to the parameters
    char inmsg5[length + 1];
    for (int i = 0; i < length; i++) {
      inmsg5[i] = (char)payload[i];
    }
    float kp, ti, td;
    char yyyy[50];
    snprintf(yyyy,50, "%f;%f;%f;%f;%i", SP, nat.kp, nat.ti, nat.td, on_off);
...
void setup() {
  ...
  Serial.begin(115200);
  setup_wifi();
  client.setServer("10.20.29.212", 1883);                                          //change the mqtt server as required by checking ifconfig
